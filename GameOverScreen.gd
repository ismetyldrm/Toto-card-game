...
func setup_results(scores: Array, isimler: Array, is_multiplayer: bool):
	...
	avg_label.add_theme_color_override("font_color", Color.YELLOW)
	...
	for i in range(4):
		...
		if is_winner:
			p_label.add_theme_color_override("font_color", Color.GREEN)
		else:
			p_label.add_theme_color_override("font_color", Color.RED)
